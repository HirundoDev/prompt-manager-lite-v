...
- ✅ Análisis de calidad de contenido
    ...
    try:
        ...
        if args.project_id:
            ...
            if not args.quiet:
                ColoredOutput.info(f"Usando proyecto: {project_info['metadata']['name']}")
        elif args.base_path:
            # Usar ruta especificada
            project_path = args.base_path
        else:
            # Intentar usar proyecto actual
            pm = ProjectManager()
            ...
            if current_id:
                ...
                if project_info:
                    project_path = project_info["path"]
                    if not args.quiet:
                        ColoredOutput.info(f"Usando proyecto actual: {project_info['metadata']['name']}")
            
            # Si no hay proyecto actual, usar directorio actual
            if not project_path:
                project_path = os.getcwd()
                if not args.quiet:
                    ColoredOutput.warning("No hay proyecto activo, usando directorio actual")
        
        # Crear generador
        generator = DailySessionGenerator(project_path)
